...
{
	double squareDist{ (pow(p.x - e.center.x, 2) / pow(e.radiusX, 2) + pow(p.y - e.center.y, 2) / pow(e.radiusY, 2)) };
	return (squareDist <= 1);
}
...
bool utils::IsOverlapping(const std::vector<Point2f>& vertices, const Rectf& r)
{

	std::vector<Point2f> rectVertices;
	rectVertices.push_back(Point2f(r.left, r.bottom));
	rectVertices.push_back(Point2f(r.left + r.width, r.bottom));
	rectVertices.push_back(Point2f(r.left + r.width, r.bottom+r.height));
	rectVertices.push_back(Point2f(r.left, r.bottom+r.height));

	for (int i{}; i < rectVertices.size(); i++)
	{
		if(utils::IsPointInPolygon(rectVertices[i], vertices)) return true;
	}

	for (int i{}; i < vertices.size(); i++)
	{
		if (utils::IsPointInPolygon(vertices[i], rectVertices)) return true;
	}
...
std::string utils::GetAttributeValue(const std::string& attrName, const std::string& element)
{
	...
	const size_t attrPos{ element.find(attrName + "=") };
	...
	const size_t attrStart{ element.find('"', attrPos) };
	const size_t attrEnd{ element.find('"', attrStart + 1) };
	const size_t count{ attrEnd - attrStart - 1 };

	std::string attribute{ element.substr(attrStart + 1, count) };

	return attribute;
}

void utils::CalculateSrcRect(const Texture* texture, Rectf& srcRect, int nrCols, int nrRows, int frame)
{
	srcRect.width = texture->GetWidth() / nrCols;
	srcRect.height = texture->GetHeight() / nrRows;
	srcRect.left = (frame % nrCols) * srcRect.width;
	srcRect.bottom = (frame / nrCols + 1) * srcRect.height;
}

void utils::RotateObjectToTarget(const Point2f& object, float& angle, const Point2f& target, float step)
{
	Point3f v1{ cosf(angle) * 5, sinf(angle) * 5, 0 };
	Point3f v2{ target.x - object.x, target.y - object.y, 0 };
	float difference{ acosf(DotProduct(v1, v2)/(GetLength(v1)*GetLength(v2))) };
	if (difference < step)step = difference;

	if (CrossProduct(v1, v2).z < 0)angle -= step;
	else angle += step;
}

float utils::GetLength(Point3f& v1)
{
	return sqrtf(powf(v1.x, 2) + powf(v1.y, 2) + powf(v1.z, 2));
}

float utils::DotProduct(Point3f& v1, Point3f& v2)
{
	return v1.x * v2.x + v1.y * v2.y + v1.z * v2.z;
}

Point3f utils::CrossProduct(Point3f& v1, Point3f& v2)
{
	Point3f result{};
