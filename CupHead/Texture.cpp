...
void Texture::Draw( const Point2f& dstBottomLeft, const Rectf& srcRect, int mode) const
{
	...
	else
	{
		...
		Draw( dstRect, srcRect, mode );
	}
}

void Texture::Draw( const Rectf& dstRect, const Rectf& srcRect, int mode) const
{
	const float epsilon{ 0.001f };
	if ( !m_CreationOk )
	{
	...
	glBindTexture( GL_TEXTURE_2D, m_Id );
	if (mode == 0) glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);
	else if (mode == 1) glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_ADD);
	else if (mode == 2) glTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);
