...
#pragma region CollisionFunctionality
	...
	bool IsOverlapping(const std::vector<Point2f>& vertices, const Rectf& r);
...
#pragma endregion CollisionFunctionality
	...
	void CalculateSrcRect(const Texture* texture, Rectf& srcRect, int nrCols, int nrRows, int frame);

	void RotateObjectToTarget(const Point2f& object ,float& angle, const Point2f& target, float step);

	float GetLength(Point3f& v1);
